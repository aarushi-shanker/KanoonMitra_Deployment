...
const model = genAI.getGenerativeModel({
    ...
    systemInstruction: "You will generate complete professional document content in detail based on the given format. Match the tone of the format. Please provide a meaningful and coherent document based on the above input. Generate important clauses. Output should be like { \"format\" : selectedFormat , \"clauses\" : {\n\"clause_heading\": heading, \"clause_content\": clause content, \"footer\": footer}",
  });
  ...
  try {
    const result = await chatSession.sendMessage("You will generate complete professional document content in detail based on the given format. Match the tone of the format. Please provide a meaningful and coherent document based on the above input. Generate important clauses. Output should be like { \"format\" : selectedFormat , \"clauses\" : {\n\"clause_heading\": heading, \"clause_content\": clause content, \"footer\": footer}");
...
const modelController = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send({ success: false, message: 'Error generating clauses', error: error.message });
  }
}
